...
- [x] 2. `poke-type-quiz` のプロジェクト画像を `public/` に追加する(配置済みだった。2026-07-13確認)
...
- [x] 7. [B-8] Profileセクションと背景画像を差別化する(ブランドネイビーのグラデーション背景に変更)
- [x] 8. [D-12] 情報設計の方針を決定する → **ワンページに統一**(2026-07-13決定。プロジェクト詳細ページのみ独立ルートとして維持)
- [x] 9. [D-12] 決定した方針に沿ってヘッダー・ページ構成を修正する
- [x] 10. [D-13] リンクの不整合を修正する(フッターへのQiita追加、プロフィール文末「Contactから〜」のリンク化、ブログアイコンの見直し)
- [x] 11. [D-13] Outputsにブログ・noteの最新記事をRSSで自動表示する(YouTubeと同様の仕組み)
...
## バックログ: 今後の宿題(2026-07-13時点)

フェーズ10〜12の改善は完了。以下は将来のタイミングが来たら着手する。

- [ ] 1. 「Services(事業内容)」セクションの新設(フェーズ11-8のスキップ分)。受注実績が増えてきたタイミングで、受注したい3領域(AI業務効率化 / Remotion動画制作 / 資産形成支援)を料金感・依頼の流れとともに掲載する **(要ヒアリング: 掲載内容)**
- [ ] 2. 実績数字が育ったら「チャンネル登録者数」「ブログ月間PV」バッジを再表示する。取得ロジックは実装済みで、`src/components/HomePageClient.tsx` の `StatsBadges` に `subscriberCount` / `blogMonthlyPv` のpropsを渡すだけ(Outputsのチャンネルヘッダーへの登録者数表示も同様に復活可能)。目安: 登録者1,000人 or 月間1万PV
- [ ] 3. 経歴(`_contents/career/6.md`)にテックリード経験を追記する。車載システム開発でのテックリードとしての部下指導・チームリーダーとしてのコードレビュー経験は、スキルレベル5(指導できる)の裏付けになる **(要ヒアリング: 具体的な内容)**
